...
def plot_map_with_nodes(map_instance, nodes, paths):
    ...
    for node in nodes:
        x, y = node.id
        ax.plot(y + 0.5, x + 0.5, marker='o', color='blue', markersize=5)
        for neighbor in node.neighbors:
            nx, ny = neighbor.id
            ax.plot([y + 0.5, ny + 0.5], [x + 0.5, nx + 0.5], color='gray', linestyle='--', linewidth=0.5)
    ...
    for path in paths:
        if path:
            for i in range(len(path) - 1):
                ...
                ax.plot([start[1] + 0.5, end[1] + 0.5], [start[0] + 0.5, end[0] + 0.5], color='red', linewidth=2)
